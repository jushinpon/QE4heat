...
my @all_files = `grep -v '^[[:space:]]*\$' $currentPath/QEjobs_status/Queueing.txt`;#all queuing QE cases
map { s/^\s+|\s+$//g; } @all_files;
die "No Queueing.txt in $currentPath/QEjobs_status" unless(@all_files);
my $submitJobs = "no";
my %sbatch_para = (
